...
| 唤醒词检测 | 实时检测指定唤醒词，触发自动转录 | WhisperHotword, PorcupineVAD |
...
唤醒词功能允许用户通过语音指令（如"小爱同学"）自动触发转录，无需手动操作。该功能采用双模式设计，兼顾灵活性和用户体验：

1. **持续监听模式**：客户端持续监听麦克风输入，实时分析音频流
2. **唤醒词检测**：使用 Porcupine VAD 引擎检测预设的关键词
3. **自动转录触发**：检测到唤醒词后，自动启动转录客户端
4. **智能超时机制**：用户停止说话后，自动等待指定时间后结束转录
5. **无缝切换**：转录结束后自动恢复监听状态，等待下一次唤醒词

工作流程：
1. 用户启动唤醒词监听模式
2. 系统持续监听麦克风，不进行转录
3. 当检测到预设唤醒词（如"小爱同学"）时
4. 系统自动启动转录客户端，开始实时转录
5. 用户说话内容被实时转录显示
6. 用户停止说话超过设定时间（默认6秒）后，自动结束转录
7. 系统恢复唤醒词监听状态，等待下一次触发

前端 SDK 提供 `WhisperHotword` 类，封装完整的唤醒词检测和自动转录功能，开发者只需简单配置即可集成。
...
唤醒词检测采用高效的 Porcupine VAD 引擎，结合自定义的音频处理流程，实现低功耗的实时关键词检测：

#### 2.5.1 算法原理

1. **关键词模型**：使用预训练的 Porcupine 模型，支持自定义唤醒词训练
2. **实时音频流处理**：持续监听16kHz采样率的音频流，分块处理
3. **特征提取**：提取MFCC特征，输入到唤醒词检测模型
4. **置信度评分**：模型输出唤醒词检测的置信度分数
5. **阈值判定**：超过预设阈值即判定为检测到唤醒词
...
class WhisperHotword:
    def __init__(self, server_url, wakeword="小爱同学"):
        self.server_url = server_url
        self.wakeword = wakeword
        self.is_listening = False
        self.is_transcribing = False
        self.auto_stop_timeout = 6000  # 6秒自动停止
    ...
    def _handle_audio_chunk(self, audio_data):
        # 音频分块处理
        audio_chunk = self._preprocess_audio(audio_data)
        # 唤醒词检测
    ...
    def _start_transcription(self):
        # 创建转录客户端
        self.transcription_client = WhisperAsr(self.server_url)
        # 绑定回调函数
        self.transcription_client.onresult = self._handle_transcription_result
        # 开始转录
...
1. **低功耗监听**：仅在检测到语音活动时才进行完整处理
2. **内存优化**：采用环形缓冲区，避免内存泄漏
3. **实时性保证**：音频分块处理，确保低延迟检测
4. **错误恢复**：网络中断或处理错误时自动重连

#### 2.5.4 可配置参数

- **唤醒词选择**：支持多种预设唤醒词，可自定义训练
- **灵敏度调整**：可调节检测灵敏度，平衡误报和漏报
- **超时设置**：可配置转录自动停止时间
- **音频设备选择**：支持指定麦克风设备

#### 2.5.5 与转录系统的集成

1. **独立WebSocket连接**：唤醒词检测使用独立的 `/hotword` 端点
2. **状态同步**：检测到唤醒词后，自动切换到转录模式
3. **资源共享**：复用音频捕获资源，减少初始化开销
4. **无缝切换**：转录结束后自动恢复监听，无需用户干预
...
- **WebSocket 端点**：
  - `/asr`：按钮转录模式，用户手动控制录音
  - `/hotword`：唤醒词模式，持续监听并自动触发转录
- **通信协议**：基于 JSON 的消息格式
- **消息类型**：
  - `config`：服务器配置信息
  - `ready_to_stop`：处理完成信号
  - `hotword` / `wakeword_detected`：唤醒词检测成功消息
  - 转录结果：包含文本、说话人信息、翻译等
...
3. **唤醒词功能开发**：设计并实现唤醒词检测与自动转录功能
4. **前端界面优化**：改进实时显示效果，提升用户体验

### 5.2 具体实现的功能

1. **VAD 优化**：改进语音活动检测算法，减少误判
2. **唤醒词检测**：实现基于 Porcupine 的实时唤醒词检测功能
3. **自动转录触发**：设计唤醒词检测到转录的无缝切换机制
4. **并行处理**：实现转录、说话人识别和翻译的并行处理
5. **前端实时显示**：优化前端渲染逻辑，实现平滑的实时更新
6. **多后端支持**：支持多种 Whisper 后端，包括 MLX、Faster-Whisper 等

### 5.3 解决的技术难题

1. **延迟优化**：通过 Simul-Streaming 策略和 AlignAtt 算法，将延迟降低到最低
2. **唤醒词检测精度**：平衡唤醒词检测的灵敏度和误报率，实现高准确率检测
3. **状态无缝切换**：解决唤醒词监听与转录模式之间的无缝切换技术问题
4. **多语言支持**：集成 NLLW 模型，实现 200+ 语言的实时翻译
5. **说话人识别**：集成 Sortformer 模型，提高说话人识别的准确性和实时性
6. **资源管理**：优化内存使用，支持多用户并发处理
...
5. **唤醒词功能开发**：
   - 实现 `WhisperHotword.js` 前端唤醒词客户端
   - 开发 `WhisperAsr.js` 中的 `TranscriptRenderer` 实时渲染组件
   - 创建唤醒词检测 WebSocket 端点 (`/hotword`)
   - 优化音频流处理，支持双模式切换（监听/转录）
