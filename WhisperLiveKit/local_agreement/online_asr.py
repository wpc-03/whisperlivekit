...
class OnlineASRProcessor:
    ...
    def __init__(
        ...
        self.init_prompt = getattr(asr, 'init_prompt', None)
        self.static_init_prompt = getattr(asr, 'static_init_prompt', None)
        self.keywords = getattr(asr, 'keywords', [])
        self.keywords_string = getattr(asr, 'keywords_string', '')
        self.init()
    ...
    def prompt(self) -> Tuple[str, str]:
        ...
        Returns a tuple: (prompt, context), where:
          - prompt is a 200-character suffix of committed text that falls 
            outside the current audio buffer, combined with static_init_prompt.
        ...
        # Combine with static_init_prompt if available
        # 注释掉动态上下文的添加
        # dynamic_prompt = self.asr.sep.join(prompt_list[::-1])
        dynamic_prompt = ""
        # Priority: keywords -> init_prompt -> empty
        base_prompt = ""
        if self.keywords_string:
            base_prompt = self.keywords_string
        elif self.init_prompt:
            base_prompt = self.init_prompt
        
        # If no dynamic prompt yet, use base_prompt
        if not dynamic_prompt:
            dynamic_prompt = base_prompt
        elif base_prompt:
            # If we have both, combine them
            dynamic_prompt = base_prompt + self.asr.sep + dynamic_prompt
        
        # Add static_init_prompt at the beginning if available
        if self.static_init_prompt:
            if dynamic_prompt:
                final_prompt = self.static_init_prompt + self.asr.sep + dynamic_prompt
            else:
                final_prompt = self.static_init_prompt
        else:
            final_prompt = dynamic_prompt
            
        return final_prompt, context_text
