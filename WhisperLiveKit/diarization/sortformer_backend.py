...
class SortformerDiarization:
    ...
    def _load_model(self, model_name: str):
        ...
        try:
            ...
            # Check if model_name is a local .nemo file
            if model_name.endswith('.nemo') and os.path.isfile(model_name):
                # Load from local .nemo file
                logger.info(f"Loading Sortformer model from local .nemo file: {model_name}")
                ...
                # Look for .nemo files in snapshots subdirectory
                nemo_files = []
                ...
                if nemo_files:
                    # Use the first .nemo file found
                    nemo_path = nemo_files[0]
                    logger.info(f"Loading Sortformer model from cache directory: {nemo_path}")
                    self.diar_model = SortformerEncLabelModel.restore_from(
                    ...
                    # Try from_pretrained for HuggingFace model ID
                    logger.info(f"Loading Sortformer model from HuggingFace: {model_name}")
